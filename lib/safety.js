...
export function safeCustomerReply(text, opts = {}) {
  ...
  if (v.ok) return text;
  // Safe fallbacks — must themselves pass the filter.
  const fallbacks = {
...
export function safeRecommendedAction(text, opts = {}) {
  const v = checkAll(text);
  if (v.ok) return text;
  // Replace with a neutral instruction.
  return "Review the case in the operations console and escalate to the appropriate department per the safety policy.";
}
