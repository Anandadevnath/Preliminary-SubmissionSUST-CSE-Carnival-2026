...
const TEMPLATES = {
  wrong_transfer: (ctx) => {
    ...
    if (lang === "bn") {
      return {
        agent_summary: `গ্রাহক ${tx} এর মাধ্যমে ${amt} ভুল প্রাপকের কাছে পাঠিয়েছেন বলে জানিয়েছেন। প্রমাণ: ${verdictWord(verdict, "bn")}।`,
        recommended_next_action: `${tx} এর বিবরণ যাচাই করুন এবং প্রাপকের সাথে যোগাযোগের চেষ্টা করুন। বিভাগ: dispute_resolution।`,
    ...
    return {
      agent_summary: `Customer reports sending ${amt} to the wrong recipient via ${tx}. Evidence verdict: ${verdictWord(verdict, "en")}.`,
      recommended_next_action: `Verify ${tx} details with the customer and attempt recipient contact through official channels. Route to dispute_resolution.`,
  ...
  payment_failed: (ctx) => {
    ...
    if (lang === "bn") {
      return {
        agent_summary: `গ্রাহক ${tx} ব্যর্থ হয়েছে বলে জানিয়েছেন কিন্তু ${amt} বাদ দেওয়া হয়েছে। প্রমাণ: ${verdictWord(verdict, "bn")}।`,
        recommended_next_action: `${tx} এর ব্যর্থতার কারণ যাচাই করুন এবং প্রয়োজনে অটো-রিভার্সাল স্ট্যাটাস পরীক্ষা করুন। বিভাগ: payments_ops।`,
    ...
    return {
      agent_summary: `Customer reports ${tx} failed but ${amt} was deducted. Evidence verdict: ${verdictWord(verdict, "en")}.`,
      recommended_next_action: `Verify ${tx} failure reason and check auto-reversal status. Route to payments_ops for manual reconciliation if needed.`,
...
export function buildReplies(args) {
  ...
  const agent_summary = raw.agent_summary;
